...
host = col1.selectbox("选择主播", [h["host"] for h in hosts])
...
st.caption(f"{host} · 共 {len(rooms)} 场")
...
cached = store.load_analysis(f"{host}_auto")
hcol1, hcol2 = st.columns([3, 1])
hcol1.subheader("🧠 整体复盘报告")
hcol1.caption("漏斗逐级诊断 + 流量/转化/下单三维度 + benchmark 对比 + 开播/关播时机 + TODO（融合录像/脚本/数据洞察）")
if hcol2.button("🔄 重新生成", type="primary", key="regen"):
    aprog = st.empty()
    with st.spinner("综合分析中（benchmark/录像/脚本/数据洞察 → Opus 4.8）…"):
        cached = orchestrator.auto_analyze(host, gran, deep, deep, deep,
...
if cached and not cached.get("error"):
    ...
    if bench:
        st.info(f"🏆 历史最佳场：{bench.get('_open_time')} · GMV ${bench.get('gmv_usd',0):,.0f} · "
